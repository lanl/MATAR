...
int main() {
    std::cerr << "This example requires MATAR built with HAVE_MPI and HAVE_KOKKOS.\n";
    return 0;
}
...
int main(int argc, char* argv[]) {
    ...
    MATAR_INITIALIZE(argc, argv);
    {
        ...
        if (rank == 0) {
            std::cout << "all_reduce(sum): " << global_sum << " (expect " << expected_sum << ")\n";
        }
        ...
        if (rank == 0) {
            std::cout << "all_reduce(sum) with different size arrays on each rank (10*rank_id): " << global_sum << " (expect " << expected_sum
                      << ")\n";
        }
        ...
        if (rank == 0) {
            std::cout << "all_reduce(min) with different size arrays on each rank (10*rank_id): " << global_min << " (expect " << expected_min
                      << ")\n";
        }
        ...
        if (rank == 0) {
            std::cout << "all_reduce(max) with different size arrays on each rank (10*rank_id): " << global_max << " (expect " << expected_max
                      << ")\n";
        }
        ...
        if (rank == 0) {
            std::cout << "all_reduce(product): " << global_product << " (expect " << expected_product << ")\n";
        }
        ...
        // with elem_position ∈ {0,1,2} as x, y, z. Reduce over elem_id for each axis.
        {
            ...
            if (rank == 0) {
                std::cout << "all_reduce(max, coord) rank-2 centroids — max x: " << max_x << " (expect " << expect_max_x << ")\n";
        ...
        // Rank-3: reduce over e at fixed tensor component — e.g. stress(e, 0, 1).
        {
            ...
            const double expect_3d = 10000.0 * static_cast<double>(size - 1) + 1000.0 * static_cast<double>(n_elem - 1) + 1.0;
            if (rank == 0) {
        ...
        // Rank-4: reduce over element at fixed Gauss point and tensor component.
        {
            ...
            const double max_qp    = s4.all_reduce(operation::max, g_fix, ti_fix, tj_fix);
            const double expect_4d = 100000.0 * static_cast<double>(size - 1) + 1000.0 * static_cast<double>(n_elem - 1) + 101.0;
            if (rank == 0) {
                std::cout << "all_reduce(max, g, i, j) rank-4 stress(e,g,i,j): " << max_qp << " (expect " << expect_4d << ")\n";
            }
        }
