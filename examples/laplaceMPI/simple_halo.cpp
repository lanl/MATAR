...
*/
    ...
    MPIPartitionKokkos <double> partition_map = MPIPartitionKokkos <double> (neighbs, world_size, rank, halo, MPI_COMM_WORLD, "pmap");
...
void example_comms_with_map_3D(int world_size, int rank) {
    // neighbor calcs
    int n = sqrt(world_size);
    int world_i = rank % n;
    int world_j = rank / n;
...
*/
    ...
    int arr_size_k = 5;
    MPIPartitionKokkos <double> partition_map = MPIPartitionKokkos <double> (1, world_size, rank, halo, MPI_COMM_WORLD, "pmap");
    MPICArrayKokkos <double> velocity = MPICArrayKokkos <double> (arr_size_i, arr_size_j, arr_size_k, "velo");
    DCArrayKokkos <double> varX = DCArrayKokkos <double> (arr_size_i, arr_size_j, arr_size_k, "varX");
    ...
    */
    ...
    for (int ii = 0; ii < arr_size_i; ii++) {
        for (int jj = 0; jj < arr_size_j; jj++) {
            for (int kk = 0; kk < arr_size_k; kk++) {
            // if halo -1, else owned so rank
                varX.host(ii, jj, kk) = rank * rank;
                if (ii < halo || jj < halo || ii >= width_loc + halo || jj >= height_loc + halo)
                    velocity.host(ii, jj, kk) = -1.0;
                else
                    velocity.host(ii, jj, kk) = rank;
            }
        }
    }
    varX.update_device();
...
*/
    ...
    for (int ii = 0; ii < arr_size_i; ii++) {
        for (int jj = 0; jj < arr_size_j; jj++) {
            for (int kk = 0; kk < arr_size_k; kk++) {
                // right neighbor halo
                if (ii < halo && i_w >= 0)
                    velocity.host(ii, jj, kk) = rank_w;
                // right side
                if (ii >= width_loc + halo && i_e < n)
                    velocity.host(ii, jj, kk) = rank_e;
                // top side
                if (jj < halo && j_n >= 0)
                    velocity.host(ii, jj, kk) = rank_n;
                // bot side
                if (jj >= height_loc + halo && j_s < n)
                    velocity.host(ii, jj, kk) = rank_s;
            }
        }
    }
    velocity.update_device();
    ...
    FOR_ALL(ii, 1, arr_size_i-1,
             jj, 1, arr_size_j-1,
              kk, 0, arr_size_k, {
              velocity(ii, jj, kk) = varX(ii-1, jj, kk) + varX(ii+1, jj, kk) + varX(ii, jj-1, kk) + varX(ii, jj+1, kk);  
    });
    Kokkos::fence();
    FOR_ALL(ii, 1, arr_size_i-1,
             jj, 1, arr_size_j-1,
    ...
    for (int ts = 0; ts < 1; ts++) {
        ...
        FOR_ALL(ii, 0, arr_size_i,
                 jj, 0, arr_size_j,
                  kk, 0, arr_size_k, {
                    varX(ii, jj, kk) = velocity(ii, jj, kk) * 2; 
        });
        Kokkos::fence();
...
int main(int argc, char *argv[])
{
  ...
  example_comms_with_map_3D(world_size, rank);
