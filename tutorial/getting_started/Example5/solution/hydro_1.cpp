...
KOKKOS_INLINE_FUNCTION int get_corners_in_cell(int, int);  // Get corner indices for a cell

KOKKOS_INLINE_FUNCTION int get_corners_in_node(int, int);  // Get corner indices for a node

// Visualization function
void plot_density_vs_position(const CArrayDual<double>& cell_coords, const CArrayDual<double>& cell_den, const int num_cells, const char* title);
    ...
    */
    ...
    MATAR_INITIALIZE();
    {
        ...
        FOR_ALL(cell_id, 0, num_cells, {
            cell_coords(cell_id) = 0.5 * (node_coords(cell_id) + node_coords(cell_id + 1));
        ...
        FOR_ALL(cell_id, 0, num_cells, {
            ...
            for (int reg = 0; reg < num_regions; reg++) {
                if (cell_coords(cell_id) >= ics[reg].x_min && cell_coords(cell_id) <= ics[reg].x_max) {
                    cell_den(cell_id)   = ics[reg].den;
                    cell_sie(cell_id)   = ics[reg].sie;
                    cell_gamma(cell_id) = ics[reg].gamma;

                    cell_pres(cell_id) = cell_den(cell_id) * cell_sie(cell_id) * (cell_gamma(cell_id) - 1.0);

                    cell_sspd(cell_id) = sqrt(cell_gamma(cell_id) * cell_pres(cell_id) / cell_den(cell_id));
        ...
        double total_e = 0.0;
        double e_lcl   = 0.0;
        FOR_REDUCE_SUM(cell_id, 0, num_cells,
                       e_lcl, {
            e_lcl += cell_mass(cell_id) * cell_sie(cell_id) + 0.5 * cell_mass(cell_id) * 0.5 * pow(node_vel(cell_id), 2) +
        ...
        for (int cycle = 0; cycle < max_cycles; cycle++) {
            ...
            double dt_lcl;
            double min_dt_calc;
            FOR_REDUCE_MIN(cell_id, 0, num_cells,
                           dt_lcl, {
                // mesh size
            ...
            for (int rk_stage = 0; rk_stage < num_rk_stages; rk_stage++) {
                ...
                FOR_ALL(cell_id, 0, num_cells, {
                    ...
                    if (dvel < 0.0) {
                        ...
                        visc = 1.0 / 2.0 * cell_den(cell_id) * cell_sspd(cell_id) * fabs(dvel) + 1.2 / 4.0 * cell_den(cell_id) * pow(dvel, 2.0);
                    ...
                    // left corner force
                    corner_force(corner_id_0) = integral_grad_basis[0] * (-cell_pres(cell_id) - visc);

                    // right corner force
                    corner_force(corner_id_1) = integral_grad_basis[1] * (-cell_pres(cell_id) - visc);
                });  // end parallel for on device
                ...
                FOR_ALL(node_id, 1, num_nodes - 1, {
                    ...
                    double force_tally = corner_force(corner_id_0) + corner_force(corner_id_1);

                    // update velocity
                    node_vel(node_id) = node_vel_n(node_id) + rk_alpha * dt / node_mass(node_id) * force_tally;
                });  // end parallel for on device
                ...
                FOR_ALL(node_id, 0, num_nodes, {
                    // velocity at t+1/2
                    double half_vel = 0.5 * (node_vel_n(node_id) + node_vel(node_id));

                    // update velocity
                    node_coords(node_id) = node_coords_n(node_id) + rk_alpha * dt * half_vel;
                });  // end parallel for on device
                ...
                FOR_ALL(cell_id, 0, num_cells, {
                    ...
                    double half_vel_0 = 0.5 * (node_vel_n(cell_id) + node_vel(cell_id));
                    double half_vel_1 = 0.5 * (node_vel_n(cell_id + 1) + node_vel(cell_id + 1));

                    double power_tally = corner_force(corner_id_0) * half_vel_0 + corner_force(corner_id_1) * half_vel_1;

                    // update specific interal energy
                    cell_sie(cell_id) = cell_sie_n(cell_id) - rk_alpha * dt / cell_mass(cell_id) * power_tally;
        ...
        FOR_REDUCE_SUM(cell_id, 0, num_cells,
                       ef_lcl, {
            ef_lcl += cell_mass(cell_id) * cell_sie(cell_id) + 0.5 * cell_mass(cell_id) * 0.5 * pow(node_vel(cell_id), 2) +
                      0.5 * cell_mass(cell_id) * 0.5 * pow(node_vel(cell_id + 1), 2);
        }, total_e_final);
        MATAR_FENCE();
...
KOKKOS_INLINE_FUNCTION int get_corners_in_cell(int cell_gid, int corner_lid) { return (2 * cell_gid + corner_lid); }

// Get the global index of a corner within a node
// node_gid: Global node index
// corner_lid: Local corner index (0=left, 1=right)
KOKKOS_INLINE_FUNCTION int get_corners_in_node(int node_gid, int corner_lid) { return (2 * node_gid - 1 + corner_lid); }
