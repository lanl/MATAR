...
enum class communication_plan_type { no_communication, all_to_all_graph };

/// Reduction operation for @c MPICArrayKokkos::all_reduce (maps to @c MPI_Op for the MPI stage).
enum class operation { sum, product, max, min };
...
struct CommunicationPlan {
    ...
    int* recv_weights = MPI_UNWEIGHTED;  // [size: num_recv_ranks] Weights on incoming edges, set to
                                         // MPI_UNWEIGHTED if not used

    // send_weights: Weights on outgoing edges (not used here, set to MPI_UNWEIGHTED)
    // Could be used to specify communication volume if needed for optimization
    int* send_weights = MPI_UNWEIGHTED;  // [size: num_send_ranks] Weights on outgoing edges, set to
                                         // MPI_UNWEIGHTED if not used
    ...
    // Note: In the future, we may want to allow MPI to reorder ranks for optimization by setting to
    // 1, this would allow MPI to reorder the ranks to make them physically closer on the hardware.
    // This is a good optimization for large meshes, but will require maps from MPI_comm_world rank
    // IDs to the new reordered rank IDs.
    int reorder = 0;

    DRaggedRightArrayKokkos<int> send_indices_;  // [size: num_send_ranks, num_items_to_send_per_rank] Indices of items to
                                                 // send to each rank
    DRaggedRightArrayKokkos<int> recv_indices_;  // [size: num_recv_ranks, num_items_to_recv_per_rank] Indices of items to
     ...
     * about which processes (ranks) each process will communicate with. This improves the
     * efficiency and clarity of later communication (for example, with MPI_Neighbor_alltoallv).
     *
     * This function is especially useful when the communication pattern is not all-to-all, but
     * rather a sparse subset: for instance, where each process only exchanges data with a few
     * neighbors.
     *
     * ==== Key Concepts ====
     * - MPI Communicator:  An MPI object representing a group of processes that can communicate
     * with each other. For context, "MPI_COMM_WORLD" is a communicator including all processes, but
     * a graph communicator customizes direct process connections.
     * - Rank:              Integer ID identifying a process in a communicator.
     * - Distributed Graph: MPI can represent communication as a directed sparse graph, with edges
     * from this rank to those it needs to send to, and from those it will receive from.
     *
     * ==== Parameters ====
     * @param num_send_ranks   [in] Number of ranks this process will send data to (out-neighbors).
     * @param send_rank_ids    [in] Array of size num_send_ranks; each entry is the rank of a
     * process to send to.
     * @param num_recv_ranks   [in] Number of ranks this process will receive data from
     * (in-neighbors).
     * @param recv_rank_ids    [in] Array of size num_recv_ranks; each entry is the rank of a
     ...
     *    - On return, 'mpi_comm_graph' will allow use of "neighbor" collectives
     * (MPI_Neighbor_alltoall[v], etc.), which automatically use the provided topology to
     * send/receive to only neighbors efficiently.
     *
     * 4. Marks the internal flag indicating that the graph communicator has been set up
     * ("has_comm_graph").
     *
     * ==== Example Usage ====
    ...
    void initialize_graph_communicator(int num_send_ranks, int* send_rank_ids, int num_recv_ranks, int* recv_rank_ids) {
        ...
        // See https://www.open-mpi.org/doc/v4.0/man3/MPI_Dist_graph_create_adjacent.3.php for more
        // details.
        MPI_Dist_graph_create_adjacent(mpi_comm_world,                      // Existing communicator (usually MPI_COMM_WORLD)
        ...
        // Set the internal flag indicating that we have created the MPI distributed graph
        // communicator.
        has_comm_graph = true;
    }
    ...
    void verify_graph_communicator() {
        ...
        MPI_Dist_graph_neighbors(mpi_comm_graph,
                                 indegree_out,
                                 sources_out.data(),
                                 sourceweights_out.data(),
                                 outdegree_out,
                                 destinations_out.data(),
                                 destweights_out.data());
    ...
    void verify_send_recv() {
        ...
        for (int i = 0; i < num_send_ranks; i++) {
            for (int j = 0; j < send_indices_.stride_host(i); j++) {
                ...
                if (idx < 0) {
                    std::cerr << "[rank " << rank << "] ERROR: negative send index at rank " << i << ", index " << j << ": " << idx << std::endl;
                    local_verification_passed = false;
                }
            }
        }
        ...
        for (int i = 0; i < num_recv_ranks; i++) {
            for (int j = 0; j < recv_indices_.stride_host(i); j++) {
                ...
                if (idx < 0) {
                    std::cerr << "[rank " << rank << "] ERROR: negative recv index at rank " << i << ", index " << j << ": " << idx << std::endl;
                    local_verification_passed = false;
                }
            }
        }
